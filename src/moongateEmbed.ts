...
export class MoonGateEmbed {
  ...
  async socialLogin() {
    // client id '896710466843-42ss52pj2o1j9b17477nv73smnu096e2.apps.googleusercontent.com',

  }
  async initGoogleOneTap() {
    const clientId = '896710466843-42ss52pj2o1j9b17477nv73smnu096e2.apps.googleusercontent.com'; // Make sure this is your actual client ID

    // Dynamically load the Google One Tap script
    ...
    script.onload = () => {
      ...
      google.accounts.id.initialize({
        ...
        auto_select: true // Optional: Enables automatic selection if the user is already signed in
      });
      // Prompt configuration, could be customized
      // @ts-ignore
  ...
  googleSignInPopup() {
    const clientId = '896710466843-42ss52pj2o1j9b17477nv73smnu096e2.apps.googleusercontent.com'; // Your actual client ID
    const redirectUri = "https://web.moongate.one/callback"; // Use the origin as the redirect URI
    const origin = window.location.href; // Get the current URL to return to
    const state = encodeURIComponent(origin); // Encode the origin URL to include as state
    ...
    const nonce = generateNonce(16); // Generate a nonce
  ...
  // Add this method call in your constructor or appropriate initialization method
  onRamp(url: string) {
    // check if the onramp is already open
    ...
    // Create the container for the onramp iframe
    ...
    container.style.backgroundColor = "#0D1117"; // Updated background color
    container.style.paddingTop = "45px"; // Add padding to the top


    document.body.appendChild(container);

    // Create the onramp iframe
    ...
    closeButton.style.top = "5px"; // Position it above the container
    ...
    // Add transition effect to the overlay
    setTimeout(() => {
